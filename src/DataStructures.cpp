...
DataStructures::DataStructures(vector<int> &logfr_, int ns_, double **coor_, int ntet_, int npyr_,
	...
	: logfr(logfr_), ns(ns_), coor(coor_), ntet(ntet_), npyr(npyr_),
	  npri(npri_), nhex(nhex_), nall(nall_), nu(nu_)
{
...
void DataStructures::setperiph()
{
	...
	for (unsigned int idum = listnp1[1]; idum <= listnp2[1]; idum++)
	{
		is = listn[idum];
		if (is < 0)
		{
		...
		for (unsigned int idum1 = idum + 1; idum1 <= listnp2[1]; idum1++)
		{
			...
			if (drr < eps && dzz < eps)
			{
				...
				{
					pint = (x1 * x2 + y1 * y2) / r12;
					pext = (x1 * y2 - x2 * y1) / r12;
					phi = abs(acos(pint));
					if (phi > phimax)
						phimax = phi;
					if (phi < phimin)
...
void DataStructures::volumes()
{
	...
	for (unsigned int ip = ntet + 1; ip <= ntet + npyr; ip++)
	{ // check orientation
		is1 = nu[ind + 1];
		is2 = nu[ind + 2];
		is3 = nu[ind + 3];
