...
void ioGrids::vtk_graphics_3D_unstr(double **coorp)
{
	...
	for (int i = 0; i < gInfo.ns; i++)
	{
		vtk << coorp[0][i] << "		" << coorp[1][i] << "		" << coorp[2][i] << endl;
	}

	int nsize = 5 * gInfo.ntet + 6 * gInfo.npyr + 7 * gInfo.npri + 9 * gInfo.nhex;

	int ntype1 = 10;
	vtk << "CELLS " << gInfo.nall << " " << nsize << endl;

	int kounter = 1;
	for (int i = 0; i < gInfo.ntet; i++)
	{
		vtk << 4 << " " << gInfo.nu[kounter] - 1 << " " << gInfo.nu[kounter + 1] - 1
			<< " " << gInfo.nu[kounter + 2] - 1 << " " << gInfo.nu[kounter + 3] - 1 << endl;
		kounter += 4;
	}

	for (int i = 0; i < gInfo.npyr; i++)
	{
		vtk << 5 << " " << gInfo.nu[kounter] - 1 << " " << gInfo.nu[kounter + 1] - 1
			<< " " << gInfo.nu[kounter + 2] - 1 << " " << gInfo.nu[kounter + 3] - 1 << " " << gInfo.nu[kounter + 4] - 1 << endl;
		kounter += 5;
	}

	for (int i = 0; i < gInfo.npri; i++)
	{
		vtk << 6 << " " << gInfo.nu[kounter] - 1 << " " << gInfo.nu[kounter + 1] - 1
			<< " " << gInfo.nu[kounter + 2] - 1 << " " << gInfo.nu[kounter + 3] - 1 << " " << gInfo.nu[kounter + 4] - 1
			<< "	" << gInfo.nu[kounter + 5] - 1 << endl;
		kounter += 6;
	}

	for (int i = 0; i < gInfo.nhex; i++)
	{
		vtk << 8 << " " << gInfo.nu[kounter] - 1 << " " << gInfo.nu[kounter + 1] - 1
			<< " " << gInfo.nu[kounter + 2] - 1 << " " << gInfo.nu[kounter + 3] - 1 << " " << gInfo.nu[kounter + 4] - 1
			<< "	" << gInfo.nu[kounter + 5] - 1 << " " << gInfo.nu[kounter + 6] - 1 << " " << gInfo.nu[kounter + 7] - 1 << endl;
		kounter += 8;
	}

	vtk << "CELL_TYPES " << gInfo.nall << endl;
	for (int i = 0; i < gInfo.ntet; i++)
	{
		vtk << 10 << endl;
	}
...
void ioGrids::checkPitch(double **coorp_, int **iper, double pitch, double nper)
{
